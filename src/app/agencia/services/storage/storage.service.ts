...
export class StorageService {
 ...
 private agencias: Agencia[] = [{
  ...
  /*
    Validamos la data al ingresar por primera vez,
    si no existe, la almacenamos,
  ...
  */
  ...
  /*Lista todas las agencias del local storage*/ 
  ListAgencias():Array<Agencia>{
    this.CheckStorage();
    return JSON.parse(localStorage.getItem("agencias")!);
  }
 ...
 private saveAgencias(){
  ...
  this.agencias.map(function(agencia:Agencia){
        agencia.id=contador;
        contador++;
        agencia.imagen="../../../../assets/imagenes/agencia"+ Math.ceil( Math.random()*10)+".jpg"
  })
  
   localStorage.setItem('agencias',JSON.stringify(this.agencias))
}

  getAgencia(id:any):Agencia{
     return this.ListAgencias().find(agencia=>{
      return agencia.id== parseInt(id)
    })!;
  }

  updateAgencia(agencia:Agencia){

    let _agencias =  this.ListAgencias().map(element =>{
      if (element.id==agencia.id){
        return agencia;
      }else {
        return element
      };
    ...
    localStorage.setItem("agencias",JSON.stringify(_agencias));
    return true;
  }


  searchItem(sucursal:string):Array<Agencia>{
    return this.ListAgencias().filter(agencia=>{
      return agencia.agencia.toUpperCase().includes(sucursal.toUpperCase());
   });
  }
